...
export function decorateLinks(block) {
  [...block.querySelectorAll('a')]
    ...
    .forEach((link) => {
      ...
      const external = !url.host.match('bahamas.macktrucks.com') && !url.host.match('.hlx.(page|live)') && !url.host.match('localhost');
      if (url.host.match('build.bahamas.macktrucks.com') || url.pathname.endsWith('.pdf') || external) {
        link.target = '_blank';
      }
    });
}
